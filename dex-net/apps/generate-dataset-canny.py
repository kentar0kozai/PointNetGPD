...
def worker(i, sample_nums, grasp_amount, good_grasp):
    ...
    while np.sum(good_count_perfect < minimum_grasp_per_fc) != 0:
        grasps = ags.generate_grasps(obj, target_num_grasps=sample_nums, grasp_gen_mult=10, vis=False, random_approach_angle=True)
        count += len(grasps)
        for j in grasps:
            ...
            for ind_, value_fc in enumerate(fc_list):
                ...
                if tmp and not is_force_closure:
                    if good_count_perfect[ind_ - 1] < minimum_grasp_per_fc:
                        canny_quality = PointGraspMetrics3D.grasp_quality(j, obj, canny_quality_config[fc_list[ind_ - 1]], vis=False)
    ...
    print("Gripper:{} Object:{} Rate:{:.4f} {}/{}".format(gripper_name, object_name_, good_grasp_rate, len(good_grasp), count))
